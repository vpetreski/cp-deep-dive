...
# repo at .claude/memory for git-based versioning.
#
# Why: the previous symlink trick (setup-memory-link.sh, deprecated) caused
# permission prompts because writes through the symlink resolved into the
# project working tree, hitting the protected-edit-path check that
# --dangerously-skip-permissions doesn't override. Memory now lives at
# the canonical location (outside the project tree, no prompts) and is
# mirrored into the repo via this post-commit hook for versioning.
#
...
echo "  No permission prompts — memory writes happen at the canonical path"
echo "  outside the project working tree."
